    ...
    where TKey : struct, IComparable<TKey>
{
    ...
    [NotNullIfNotNull(nameof(Stream))]
    ...
    public static FwobFile<TFrame, TKey> CreateNewFile(
        string path,
        string title,
    ...
    public static void Concat(string dstPath, params string[] srcPaths)
    {
        ...
        try
        {
            ...
            foreach (string path in srcPaths)
            {
                ...
                Debug.Assert(file.Strings != null);
            ...
            Debug.Assert(patFile != null);
            Debug.Assert(patFile.Stream != null);
    ...
    public FwobFile(string path, FileAccess fileAccess = FileAccess.ReadWrite, FileShare fileShare = FileShare.Read)
    {
        ...
        var frameInfo = header.GetFrameInfo<TFrame>();
        if (frameInfo == null)
    ...
    private static Action<BinaryWriter, TFrame> GenerateFrameSerializer()
    {
        ...
        foreach (var fieldInfo in typeof(TFrame).GetFields())
        {
            ...
            if (length > 0) // string
            {
                ...
                var lengthPropType = typeof(string).GetProperty(nameof(string.Length));

                Debug.Assert(lengthPropType != null);
                var lengthProp = Expression.Property(fieldExp, lengthPropType); // frame.{field}.Length
                ...
                var exceptionCtor = typeof(KeyOrderingException).GetConstructor(new[] { typeof(string) });

                Debug.Assert(exceptionCtor != null);
                var exceptionExp = Expression.New(exceptionCtor, exceptionMsg); // new KeyOrderingException(...)
                ...
                var padRightMethod = typeof(string).GetMethod(nameof(string.PadRight), new[] { typeof(int) });
                ...
                var toCharArrayMethod = typeof(string).GetMethod(nameof(string.ToCharArray), Array.Empty<Type>());
            ...
            var writeMethod = typeof(BinaryWriter).GetMethod(nameof(BinaryWriter.Write), new[] { fieldType })
    ...
    private static Func<BinaryReader, TFrame> GenerateFrameDeserializer()
    {
        ...
        foreach (var fieldInfo in typeof(TFrame).GetFields())
        {
            ...
            if (length > 0) // string
            {
                var readMethod = typeof(BinaryReader).GetMethod(nameof(BinaryReader.ReadChars), new[] { typeof(int) });
                ...
                var trimEndMethod = typeof(string).GetMethod(nameof(string.TrimEnd), Array.Empty<Type>());
